...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  const getTodoArray = `SELECT * FROM todo WHERE id = "${todoId}"`;
  const todoArray = await db.get(getTodoArray);
...
app.put("/todos/:todoId/", async (request, response) => {
  ...
  const putTodoQuery = `UPDATE todo SET todo = "${todo}", priority = "${priority}", status = "${status}" WHERE id = "${todoId}"`;
